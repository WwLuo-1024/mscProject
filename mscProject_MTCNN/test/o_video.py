...
if __name__ == '__main__':
    ...
    rnet.eval()
    onet.eval()
    pnet.load_state_dict(torch.load(r'D:\face_recognition\params\p_params.pkl')) # 导入训练参数
    rnet.load_state_dict(torch.load(r'D:\face_recognition\params\r_params.pkl'))  # 导入训练参数
    onet.load_state_dict(torch.load(r'D:\face_recognition\params\o_params.pkl'))  # 导入训练参数

    cap = cv2.VideoCapture(r"D:\face_recognition\test\test.mp4")
    count = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if ret:
